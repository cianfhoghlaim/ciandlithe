...
"""ciandlithe — Students' Union Google ADK case studies (marimo notebook).

5 case studies + 1 root orchestrator, all from
`agents/adk/students_union/`. Run with:

    marimo edit notebooks/students_union_adk_case_studies.py

The notebook imports the local `agents.adk.students_union` package
...
def _imports():
    ...
    # The SU agents live at <repo>/agents/adk/students_union. Add the
    # nearest sys.path entry that lets `import agents.adk.students_union`
...
def _tools_only_check(
    ...
    mo.md(
        ...
        `python3 agents/adk/students_union/_smoke_test.py` first to
