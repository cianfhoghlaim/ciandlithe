...
"""ciandlithe — Students' Union × University of Galway integration (marimo notebook).

The 5 SU ADK agents in `~/dev/ciandlithe/agents/adk/students_union/`
consume the University of Galway public-document data emitted by the
5 DLT sources in `~/dev/kings_college_galway/dlt_sources/uog/`.
...
def _bootstrap_paths():
    ...
    # Both repos must be on sys.path for the cross-repo import to work.
    # The notebook lives at ~/dev/ciandlithe/notebooks/, so:
    #   - CIANDLITHE_REPO_ROOT = notebooks/..
    #   - KCG_REPO_ROOT        = notebooks/../../../kings_college_galway/
    notebook_dir = Path(__file__).resolve().parent
    ciandlithe_root = notebook_dir.parent
    kcg_root = ciandlithe_root.parent / "kings_college_galway"

    for p in [str(ciandlithe_root), str(kcg_root)]:
        if p not in sys.path:
            sys.path.insert(0, p)

    return ciandlithe_root, kcg_root, notebook_dir, sys
...
def _load_su_package(bootstrap_paths, stub_optional_deps):
    from agents.adk.students_union import (
        root_agent,
        students_union_app,
        classify_su_query,
        clubs_socs_agent,
    ...
    from agents.adk.students_union.tools import (
        ClubApplication,
        GrantApplication,
        Complaint,
        Candidate,
